define_actor :vine_seed do
  ...
  behavior do
    ...
    helpers do
      def range; 8 end
      def grow
        immediate_neighbors = world.occupants_for_box(actor.x-1,actor.y-1,2,2)
        unless immediate_neighbors.any?{|neighbor| neighbor.actor_type == :water_seed}

          others = world.occupants_for_box(actor.x - range, actor.y - range, actor.x + range, actor.y + range)
          ...
          unless waters.empty?
            ...
            unless closest.nil?
              ...
              stage.create_actor(actor.actor_type, x: x , y: y) unless occupant
            end
          end
        end
